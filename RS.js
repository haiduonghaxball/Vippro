...
var superAdminCode = "BNH222"; // !admin BNH222
...
// Global Variables
// ------------------------------------------------
var roomName = "REAL SOCCER CỦA BNH";
...
room.onPlayerJoin = function(player) {	
	console.log(player.name + " VÀO PHÒNG");
	whisper("⚽       CHÀO MỪNG BẠN ĐẾN VỚI REAL SOCCER VH      ⚽", player.id, 0x61ddff, "bold", 0);
	whisper("⚽        NHẬP !help ĐỂ HIỆN CÁC LỆNH CÓ SẴN     ⚽", player.id, 0x61e7ff, "bold", 0);
...
room.onPlayerLeave = function(player) {
	displayAdminMessage();
	console.log(player.name + " THOÁT PHÒNG");
...
room.onGameStart = function(byPlayer) {
	if (map == "RSR") {
		...
		if (byPlayer == null) {
			game = new Game();	
			announce("CÓ " + gameTime + " PHÚT CHO SỰ CỐ GẮNG CỦA HAI BÊN");
		}
		else {
	...
	if (message.startsWith("!")) {
		...
		if (args[0] == "admin" && args.length == 1 && allowPublicAdmin == true) {
			if (isAdminPresent() == false) {
				...
				whisper("ĐÃ CÓ ADMIN HOẶC LỆNH !admin KHÔNG KHẢ DỤNG", player.id);
			}
		}
			...
			if (player.admin) {
				room.clearBans();
				announce("DANH SÁCH BAN ĐÃ ĐƯỢC XÓA BỞI " + player.name);
			}
			else {
			...
			if (powerShotMode == false) {
				...
				powerShotMode = false;
				announce("POWERSHOT ĐÃ BỊ TẮT BỞI " + player.name, null, 0xFF0000);
			}
		}		
		else if (args[0] == "help") {
			...
			if (superAdmins.length > 0) {
				...
				superMsg = "HIỆN TẠI KHÔNG CÓ SUPER ADMIN";
			}
	...
	if (message.startsWith("t ")) {
		...
		if (player.team == 1) {
			...
			players.forEach(function(teamPlayer) {
				room.sendAnnouncement("[ĐỘI] " + player.name + ": " + teamMsg, teamPlayer.id, 0xED6A5A, "normal", 1);
			});
		}
		if (player.team == 2) {
			var players = room.getPlayerList().filter((player) => player.team == 2);
			players.forEach(function(teamPlayer) {
				room.sendAnnouncement("[ĐỘI] " + player.name + ": " + teamMsg, teamPlayer.id, 0x5995ED, "normal", 1);
			});
		}
		if (player.team == 0) {
			var players = room.getPlayerList().filter((player) => player.team == 0);
			players.forEach(function(teamPlayer) {
				room.sendAnnouncement("[DỰ BỊ] " + player.name + ": " + teamMsg, teamPlayer.id, 0xdee7fa, "normal", 1);
			});
		}
		return false;
	}
...
function displayHelp(id, selection) {
	if (selection == null) {
		whisper("CÁC LỆNH: !rs, !rr, !bb, !powershot, !ps, !admin, !setpassword, !clearpassword, !super, !clearbans, !swap, t [team chat msg], !court, !court [hexcolor], !court reset", id, null, "small");
	}
}
...
function realSoccerRef() {
	...
	if (game.time == game.extraTimeEnd && game.lastPlayAnnounced == false) {
		announce("NHỮNG GIÂY PHÚT CUỐI CÙNG", null, null, null, 1);
...
if (parseFloat(room.getDiscProperties(0).invMass.toFixed(2)) != 1.05) {
room.setDiscProperties(0, {invMass: 1.05});
room.sendAnnouncement("SÚT NHƯ CC!!!!", game.powershotID, 0xdd3333, "bold", 2);
}
}
//=========== POWERSHOT CODE ===========
}
...
function displayAdminMessage() {
	if (isAdminPresent() == false && allowPublicAdmin == true) {
		announce("HIỆN TẠI CHƯA CÓ ADMIN: NHẬP !admin ĐỂ QUẢN ROOM");
	}
}
